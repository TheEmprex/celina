...
const FeatureScreenshot = (() => {
	...
	C.MaxDims = 4096;
	// TODO: Move settings into PT.puzzleToSvg
	C.AllowedSettings = [
		'nogrid',
		'dashedgrid',
		'hidecolours',
	...
	// Dialog
		...
		P.updateScreenshot = async function() {
			...
			let {options: {width, height, type, trim, blank, showselection}} = this;
			...
			let svg = await puzzleToSvg({width, height, trim, showselection});
			const reAllowedSettings = new RegExp(`^setting-(${C.AllowedSettings.join('|')})(?:\-.+)?$`);
			...
			Framework.showDialog({
				parts: [
					...
					{style: 'margin: 0.5rem 1rem;', children: [
						...
						{tag: 'toggle', name: 'selection', content: 'Show Selection', value: options.showselection, id: 'ssoption_showselection', style: 'color: auto;'},
						{style: 'margin: 0.25rem 0;', children: [
